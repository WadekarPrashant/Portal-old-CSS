...
const CollegeProfilePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      !Number.isInteger(Number(college.minStudentCriteria)) ||
      Number(college.minStudentCriteria) <= 0
    ) {
    ...
    if (
      !Number.isInteger(Number(college.maxCriteria)) ||
      Number(college.maxCriteria) <= 0
    ) {
  ...
  const addCourse = () => {
    if (college.branch === "Others") {
      if (
        manualCourseInput &&
        !college.coursesAvailable.includes(manualCourseInput)
      ) {
        setCollege((prevCollege) => ({
          ...prevCollege,
          coursesAvailable: [
            ...prevCollege.coursesAvailable,
            manualCourseInput,
          ],
          courseCutoffs: {
            ...prevCollege.courseCutoffs,
  ...
  const removeCourse = (index) => {
    ...
    setCollege((prevCollege) => {
      const { [courseName]: _, ...remainingCutoffs } =
        prevCollege.courseCutoffs;
      return {
        ...prevCollege,
        coursesAvailable: prevCollege.coursesAvailable.filter(
          (_, i) => i !== index
        ),
    ...
    <div className="college-profile-container">
      <div className="profile-container">
        ...
        <form onSubmit={handleSubmit} className="profile-form">
          ...
          />
          {errors.collegeCode && (
            <div className="error-message">{errors.collegeCode}</div>
          )}
          ...
          <div className="course-list">
            {college.coursesAvailable.map((course, index) => (
              ...
              >
                ...
                />
                {errors[`ST-${course}`] && (
                  <div className="error-message">{errors[`ST-${course}`]}</div>
                )}
                <input
                  type="number"
                  name={`SC-${course}`}
                ...
                />
                {errors[`SC-${course}`] && (
                  <div className="error-message">{errors[`SC-${course}`]}</div>
                )}
                <input
                  type="number"
                  name={`OBC-${course}`}
                ...
                />
                {errors[`OBC-${course}`] && (
                  <div className="error-message">{errors[`OBC-${course}`]}</div>
                )}
                <input
                  type="number"
                  name={`General-${course}`}
          ...
          <input
            ...
            required
          />
          {errors.spotRoundDates && (
            <div className="error-message">{errors.spotRoundDates}</div>
          )}
          ...
          <input
            ...
            required
          />
          {errors.minStudentCriteria && (
            <div className="error-message">{errors.minStudentCriteria}</div>
          )}
          ...
          />
          {errors.maxCriteria && (
            <div className="error-message">{errors.maxCriteria}</div>
          )}
